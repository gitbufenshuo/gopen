...
func (co *CustomObject) Update() {
	scaleX := float32(math.Sin(float64(co.GI().CurFrame)*0.0175)) + 1.1
	scaleY := float32(math.Cos(float64(co.GI().CurFrame)*0.0175)) + 1.1
	co.Transform.Scale.SetIndexValue(0, scaleX)
	co.Transform.Scale.SetIndexValue(1, scaleY)
}
